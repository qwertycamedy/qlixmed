...
import { userSel } from '@store/slices/user/userSlice';
import { formatDate } from '@hooks/formatDate';
import { formatPhoneNumber } from '@hooks/formatPhoneNum';

const MainInfo = () => {
  const { product } = useSelector(productSel);
  const { user, userAvatar } = useSelector(userSel);
  const [numShow, setNumShow] = useState(false);

  const num = formatPhoneNumber(user?.phone_number);
  const regDate = formatDate(user?.created_at, true, true, true, false, false);

  const onCopyLink = () => {
    const currentURL = window.location.href;

    // Создайте временный элемент для копирования текста в буфер обмена
  ...
  console.log(averageRating(product?.reviews));
    ...
    <MySection classNames={cl.mainInfo} innerCl={cl.inner}>
...

        {
          ...
          <div className={cl.rating}>
            <div className={cl.stars}>
              ...
              {[...Array(5 - averageRating(product?.reviews))].map((_, i) => (
                <svg
      ...
      <div className={cl.bot}>
        <SellerLink
          ...
          img={`${process.env.REACT_APP_AVATARS_URL}${userAvatar}`}
          name={user?.name}
          date={regDate}
        />
        ...
        <p className={cl.address}>{user?.address}</p>
      </div>
    </MySection>
  );
};
